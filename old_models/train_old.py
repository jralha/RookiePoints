...
from conf_test import plotConfMatrix
...
vets = wr_data.loc[wr_data[('Draft', 'Draft Year')] < 2017]
soph = wr_data.loc[(wr_data[('Draft', 'Draft Year')] > 2017) & (wr_data[('Draft', 'Draft Year')] != 2020)]
...
X_res, y_res_t = sm.fit_resample(X,tiers_vets) #This is wrong, don't resample before split, I'm doing it for the memes
X_train_t, X_test_t, y_train_t, y_test_t = train_test_split(X_res, y_res_t, test_size=0.3, stratify=y_res_t,random_state=123)
...
param_grid = {}
model_hit = GridSearchCV(est_hit, param_grid,cv=10)

X_res, y_res_t = sm.fit_resample(X,hit_vets)
X_train_h, X_test_h, y_train_h, y_test_h = train_test_split(X_res, y_res_t, test_size=0.3, stratify=y_res_t)

model_hit.fit(X_train_h,y_train_h)
preds_h = model_hit.predict(X_test_h)
cf = confusion_matrix(y_test_h,preds_h,normalize='pred')
