...
def encode_df(feat,encoder):
    ...
    out = np.concatenate([scaled,encoded,encoded,encoded,encoded],axis=1)
...
parameters = {}
model0 = GridSearchCV(estimator, parameters,cv=cv,n_jobs=-1)
...
early_stop=15
consecutive=0
for train_ix, test_ix in cv.split(X, hit_vets):
    # split the data
    X_train, X_test = X[train_ix], X[test_ix]
    y_train, y_test = hit_vets[train_ix], hit_vets[test_ix]
    ...
    if tp >= tp_best and fn <= fn_best:
        ...
        tp_best = tp
        fn_best = fn
        print('TP improved to '+str(tp_best))
        model_best = model
        conf_best = conf
        comp = (y_hat == hit_soph)
    consecutive += 1
    if consecutive >= early_stop:
        break
    scores.append(tp)

print('Average TP Rate: %.3f (%.3f)' % (np.mean(scores), np.std(scores)))
plotConfMatrix(conf_best,labels=[0,1])


#%% Predictions
pred_soph_hit = model_best.predict(X_soph)
prod_soph_prob = model_best.predict_proba(X_soph)
pred_rook_hit = model_best.predict(X_rk)
pred_rook_prob = model_best.predict_proba(X_rk)
